...
// UnmarshalJSON adds compatibility for camelCase `networkSettings` returned by XBoard.
func (c *CommonNode) UnmarshalJSON(data []byte) error {
	type alias CommonNode
	var raw struct {
		alias
	...
	*c = CommonNode(raw.alias)
	if len(c.NetworkSettings) == 0 && len(raw.NetworkSettingsCamel) != 0 {
		c.NetworkSettings = raw.NetworkSettingsCamel
	}
	return nil
}
